...
class BollingerBands {
  ...
  public:
    ...
    static ENUM_BOLLINGER_BANDS_EXT get_bollinger_extension(
        datetime time, double price, int handler) {

        BollingerBandsValues bb_values;
        bool success = get_bollinger_bands_values(
            time, handler, bb_values);

        if (success && (price > bb_values.upper_band))
            return BB_UPPER_BAND_EXT;

        if (success && (price < bb_values.lower_band))
            return BB_LOWER_BAND_EXT;
